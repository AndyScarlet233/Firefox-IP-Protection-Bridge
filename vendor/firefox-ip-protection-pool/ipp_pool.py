...
USAGE_HELPER_TIMEOUT_SECONDS = 75
USAGE_JSON_PREFIX = "__FXA_USAGE_JSON__"
...
def atomic_write_text(path: Path, content: str, mode: int = 0o644) -> None:
    ...
    tmp = Path(tmp_name)
    try:
        getattr(os, "fchmod", lambda *_: None)(fd, mode)
...
class TokenStore:
    ...
    def _usage_via_helper(self) -> ProxyUsage:
        """Use renewal credentials to query quota with an ephemeral OAuth token."""
        helper = ROOT / "refresh_tokens.py"
        command = [sys.executable, str(helper), "--usage-json"]
        try:
            completed = subprocess.run(
                command,
                cwd=str(ROOT),
                ...
                text=True,
                timeout=USAGE_HELPER_TIMEOUT_SECONDS,
                check=False,
            )
        except (OSError, subprocess.TimeoutExpired) as exc:
            raise RuntimeError(f"FxA usage helper failed: {type(exc).__name__}") from exc

        payload: dict | None = None
        for line in reversed((completed.stdout or "").splitlines()):
            line = line.strip()
            if not line.startswith(USAGE_JSON_PREFIX):
                continue
            try:
                candidate = json.loads(line[len(USAGE_JSON_PREFIX):])
        ...
        if payload is None:
            ...
            raise RuntimeError("FxA usage helper returned no usable result" + suffix)

        if not payload.get("ok"):
            code = str(payload.get("error") or "transient_error")
            status = payload.get("http_status")
            with self._lock:
                self.last_status = status if isinstance(status, int) else None
            messages = {
        ...
        if not isinstance(status_raw, int) or isinstance(status_raw, bool):
            raise RuntimeError("Guardian usage helper returned an invalid HTTP status")
        headers = payload.get("headers")
        if not isinstance(headers, dict):
            raise RuntimeError("Guardian usage helper returned invalid quota headers")
        safe_headers = {
            str(k): str(v)
            for k, v in headers.items()
            if str(k).lower().startswith("x-quota") or str(k).lower() == "retry-after"
        }
        with self._lock:
            self.last_status = status_raw

        retry_after = _retry_after_seconds(safe_headers.get("Retry-After") or safe_headers.get("retry-after"))
        quota_reset = _quota_reset_seconds(
            safe_headers.get("X-Quota-Reset") or safe_headers.get("x-quota-reset")
        )
        try:
            self._set_usage(safe_headers, require_quota=status_raw == 200)
        except ValueError as exc:
            with self._lock:
                self.quota = ProxyUsage(retry_after=retry_after)
            if status_raw == 200:
                raise RuntimeError(f"invalid Guardian usage response: {exc}") from exc
        ...
        raise RuntimeError(f"Guardian usage request failed with HTTP {status_raw}")
    ...
    def usage(self) -> ProxyUsage:
        ...
        if not fxa_token:
            if self._session_refresh_available():
                return self._usage_via_helper()
            raise RuntimeError("missing Firefox renewal credentials for Guardian usage query")
...
def relay(a: socket.socket, b: socket.socket) -> None:
    ...
    for sock in sockets:
        ...
        for option in (socket.SO_RCVBUF, socket.SO_SNDBUF):
            try:
                sock.setsockopt(socket.SOL_SOCKET, option, 512 * 1024)
            except OSError:
                pass
    try:
        while True:
...
class BackendPool:
    ...
    def candidates(self, attempts: int | None = None) -> list[ExitNode]:
        with self._lock:
            ...
            if count <= 0:
                return []
            if os.environ.get("IPP_STICKY_PRIMARY") == "1":
...
class Pool:
    ...
    def _start(
        ...
        selected.sort(key=lambda node: (
            0 if node.hostname.lower() in preferred_rank else 1,
            preferred_rank.get(node.hostname.lower(), len(preferred_rank)),
            node.country.upper(), node.city.upper(), node.stable_id,
        ))
...
def cmd_usage(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    # Keep usage JSON on one line. The move-safe frozen native host sanitizes
    # output line-by-line; compact JSON survives that compatibility layer intact.
    print(json.dumps(asdict(usage), ensure_ascii=False, separators=(",", ":")))
    return 0
