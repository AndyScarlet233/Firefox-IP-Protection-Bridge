...
USAGE_FXA_HTTP_TIMEOUT = (2.0, 4.0)
USAGE_HTTP_RETRY_BUDGET = 10.0
EX_TEMPFAIL = 75
...
def guardian_request(
    ...
    *,
    ...
    budget = max(1.0, float(retry_budget))
    ...
    for attempt in range(1, HTTP_ATTEMPTS + 1):
        ...
        try:
            ...
            if attempt == HTTP_ATTEMPTS:
                break
            delay = min(float(2 ** (attempt - 1)), budget)
            remaining = deadline - time.monotonic()
            if delay > remaining:
                break
...
def _usage_headers(headers) -> dict[str, str]:
    """Return only non-secret Guardian quota headers for the parent process."""
    selected: dict[str, str] = {}
    for key, value in (headers.items() if headers else []):
        name = str(key)
        lowered = name.lower()
        if lowered.startswith("x-quota") or lowered == "retry-after":
            selected[name] = str(value)
    return selected


def _emit_usage_payload(payload: dict[str, Any]) -> None:
    # A prefix makes parsing robust even if a bounded retry wrote a diagnostic
    # line to stdout before the final machine-readable result.
    print(USAGE_JSON_PREFIX + json.dumps(payload, ensure_ascii=False, separators=(",", ":")))


def _usage_error_payload(error: str, *, http_status: int | None = None) -> int:
    payload: dict[str, Any] = {"ok": False, "error": error}
    if http_status is not None:
        payload["http_status"] = http_status
    _emit_usage_payload(payload)
    return 1


def _query_usage_with_ephemeral_oauth() -> int:
    """Query Guardian quota without ever persisting the short-lived OAuth token."""
    try:
        acc = load_renewal_credentials(TOKENS)
    except (OSError, RenewalCredentialsError):
        return _usage_error_payload("missing_credentials")
    ...
    try:
        ...
        try:
            ...
            if status == 429:
                return _usage_error_payload("oauth_rate_limited", http_status=status)
            if status is not None and 400 <= status <= 499:
                return _usage_error_payload("reauth_required", http_status=status)
            return _usage_error_payload("transient_error", http_status=status)
        if not isinstance(access_value, str) or not access_value.strip():
            return _usage_error_payload("reauth_required")
        ...
        _emit_usage_payload(
            {
                ...
                "headers": _usage_headers(response.headers),
            }
        )
        return 0
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--usage-json",
        action="store_true",
        help="mint a temporary FxA OAuth token, query Guardian quota, then destroy it",
    )
    ...
    if args.usage_json:
        if not _remove_legacy_access_token():
            return _usage_error_payload("transient_error")
        return _query_usage_with_ephemeral_oauth()
